...
class RefineFSAFHead(nn.Module):
    """Feature Selective Anchor-Free and Refinement Head
       Add share or not share head
    Args:
        ...
        share_weight (bool): If share the weights for the cls/loc branch in head(NB: not include the stacked part)
    """
    ...
    def __init__(self,
        ...
        #the refinement part
        self.refine_use_sigmoid_cls = loss_cls.get('use_sigmoid', False)
        if self.refine_use_sigmoid_cls:
            self.refine_cls_out_channels = num_classes - 1
        else:
            self.refine_cls_out_channels = num_classes
        self.feat_channels = feat_channels
    ...
    def _init_layers(self):
        ...
        if self.share_weight:
            ...
            self.refine_reg = nn.Conv2d(self.feat_channels, 4, 3, padding=1)          
        else:
            refine_reg_convs = []
            refine_cls_convs = []
            for i in range(self.num_levels):
                refine_reg_convs.append(
                    nn.Conv2d(
                        self.feat_channels,
                        4,
                        kernel_size=3,
                        padding=1))
                refine_cls_convs.append(
                    nn.Conv2d(
                        self.feat_channels,
                        self.refine_cls_out_channels,
                        kernel_size=3,
                        padding=1))
            self.refine_reg_convs = nn.ModuleList(refine_reg_convs)
            self.refine_cls_convs = nn.ModuleList(refine_cls_convs)
            fsaf_reg_convs = []
            fsaf_cls_convs = []
            for i in range(self.num_levels):
                fsaf_reg_convs.append(
                    nn.Conv2d(
                        self.feat_channels,
                        4,
                        kernel_size=3,
                        padding=1))
                fsaf_cls_convs.append(
                    nn.Conv2d(
                        self.feat_channels,
                        self.cls_out_channels,
                        kernel_size=3,
                        padding=1))
            self.fsaf_reg_convs = nn.ModuleList(fsaf_reg_convs)
            self.fsaf_cls_convs = nn.ModuleList(fsaf_cls_convs)
    ...
    def init_weights(self):
        ...
        if self.share_weight:
            ...
            xavier_init(self.refine_cls, distribution='uniform', bias=0)
            xavier_init(self.refine_reg, distribution='uniform', bias=0)
        else:
            for m in self.refine_cls_convs:
                xavier_init(m, distribution='uniform', bias=0)
            for m in self.refine_reg_convs:
                xavier_init(m, distribution='uniform', bias=0)
            for m in self.fsaf_cls_convs:
                xavier_init(m, distribution='uniform', bias=0)
            for m in self.fsaf_reg_convs:
                xavier_init(m, distribution='uniform', bias=0)
    ...
    def forward_single(self, x):
        ...
        refine_cls_score = self.refine_cls(cls_feat)
        refine_bbox_pred = self.refine_reg(reg_feat)
        return fsaf_cls_score, fsaf_bbox_pred, refine_cls_score, refine_bbox_pred
    ...
    def forward(self, feats):
        if self.share_weight:
            ...
            # anchor free part
            for cls_feat, reg_feat, reg_conv, cls_conv in zip(cls_feats, reg_feats, self.fsaf_reg_convs,
                                                self.fsaf_cls_convs):
                fsaf_cls_scores.append(cls_conv(cls_feat))
                fsaf_bbox_preds.append(self.relu(reg_conv(reg_feat)))
            # anchor refinement part
            for cls_feat, reg_feat, reg_conv, cls_conv in zip(cls_feats, reg_feats, self.refine_reg_convs,
                                                self.refine_cls_convs):
                refine_cls_scores.append(cls_conv(cls_feat))
                refine_bbox_preds.append(reg_conv(reg_feat))
            return fsaf_cls_scores, fsaf_bbox_preds, refine_cls_scores, refine_bbox_preds
    ...
    def loss(self,
        ...
        fsaf_cls_reg_targets = self.point_target(
            fsaf_cls_scores,
            fsaf_bbox_preds,
            gt_bboxes,
            img_metas,
        ...
        #     return None
        (fsaf_labels_list, fsaf_label_weights_list, fsaf_bbox_targets_list, fsaf_bbox_locs_list,
         fsaf_num_total_pos, fsaf_num_total_neg) = fsaf_cls_reg_targets
        fsaf_num_total_samples = fsaf_num_total_pos
        fsaf_losses_cls, fsaf_losses_reg = multi_apply(
            self.fsaf_loss_single,
            fsaf_cls_scores,
            fsaf_bbox_preds,
            fsaf_labels_list,
            fsaf_label_weights_list,
            fsaf_bbox_targets_list,
            fsaf_bbox_locs_list,
        ...
        refine_cls_reg_targets = anchor_target(
            ...
            sampling=False,
            unmap_outputs=False)
        if refine_cls_reg_targets is None:
            return None
        (refine_labels_list, refine_label_weights_list, refine_bbox_targets_list, refine_bbox_weights_list,
    ...
    def get_fsaf_anchors(self, cls_scores, bbox_preds, img_metas, cfg,
                   rescale=False):
        """Get anchors based on fsaf branch's predictions

        Args:
            cls_scores (list(Tensor)): fsaf cls prediction
            bbox_preds (list(Tensor)): fsaf reg prediction
            img_metas (list[dict]): Image meta info.
            cfg (dict): config

        Returns:
            tuple: anchors of each image, valid flags of each image
            Batch x level x anchors
        """
        num_levels = len(self.feat_strides)
        assert len(cls_scores) == len(bbox_preds) == num_levels
        device = bbox_preds[0].device
        ...
        for img_id in range(len(img_metas)):
            ...
            img_shape = img_metas[img_id]['img_shape']
            scale_factor = img_metas[img_id]['scale_factor']
            mlvl_bboxes, mlvl_scores = self.get_anchors_single(cls_score_list, bbox_pred_list,
                                               mlvl_points, img_shape,
                                               scale_factor, cfg, rescale)
            # mlvl_scores might be useful
            anchor_list.append(mlvl_bboxes)
